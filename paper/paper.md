...
- A real 42-minute live webcam session was logged (`results/live_log.csv`, 6,040 frames) and one frame added as
...
- **Real-time system.** Checked on simulated frames, where individual predictions differ from the direct model output about one time in four because of framing (Section 3.9), and run live for one 42-minute single-person session; no evaluation with actual drivers, varied lighting or multiple people.
