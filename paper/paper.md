...
- A real 49-minute live webcam session was logged (`results/live_log.csv`, 7,081 frames) and one frame added as
  Figure 10 (right; Section 3.9). This confirms the pipeline and alert banner work end to end on a real camera, but
  it is one person in one room, not a systematic evaluation with drivers across conditions.
...
- **Real-time system.** Checked on simulated frames, where individual predictions differ from the direct model output about one time in four because of framing (Section 3.9), and run live for one 49-minute single-person session; no evaluation with actual drivers, varied lighting or multiple people.
